...
- **설문지 사전점검** — 내 설문지·척도가 실제 조사에서 제대로 작동할지 미리 점검합니다.
  ① 문항 표현 자동 검토(이중 질문·유도 표현·어려운 용어·보기 중복/누락·척도 라벨 불균형, **API 키 없이 동작**)
  ② 응답자 이해도 테스트(가상 이용자가 문항을 자기 말로 다시 설명 → 다르게 읽히는 문항 발견)
  ③ 척도 신뢰도·문항 변별도(Cronbach's α, 문항-총점 상관, 문항 제외 시 α, 천장·바닥 효과)
  ④ 역문항·응답 일관성(역채점 처리 오류, 일괄 응답, 상위 보기 쏠림)
- **배치 실행** — 동시 실행 수·분당 호출 수 조절, 진행률 실시간 표시, **실패분만 재실행**, 반복 실행(안정성 비교).
- **호출 한도 대응** — 429를 만나면 서버가 알려준 시간만큼 기다렸다 재시도하고, 실행 전체의 호출 속도를
  자동으로 낮춥니다. 무료 등급 키로도 끝까지 완주할 수 있습니다(실측: 10명 기준 한도 실패 2건 → 0건).
...
| 문서 | 내용 |
| --- | --- |
| [사용법.md](./사용법.md) | 화면 순서대로 따라 하는 실무자용 안내서 |
| [docs/data-model.md](./docs/data-model.md) | 데이터 구조와 처리 흐름 상세 |
| [docs/블로그-초안.md](./docs/블로그-초안.md) | 프로젝트 소개글 초안(만든 배경·설계 판단·한계) |
| [PRD.md](./PRD.md) | 최초 제품 요구사항 정의서 |
